...
public class ValidatorInfoServiceTest {
	...
	public void getValidatorsSuccessfullyReturnsFirstPage() {
		...
		result
			...
			.onSuccess(tuple -> tuple.map((cursor, list) -> {
				...
				assertEquals(VALIDATOR_3, list.get(0).getAddress());
				assertEquals(VALIDATOR_2, list.get(1).getAddress());
	...
	public void getValidatorsSuccessfullyReturnsElementsUsingCursor() {
		var validatorInfoService = setUpService();
		var result = validatorInfoService.getValidators(1, Optional.of(VALIDATOR_2));
		...
		result
			...
			.onSuccess(tuple -> tuple.map((cursor, list) -> {
				...
				assertEquals(VALIDATOR_1, list.get(0).getAddress());
	...
	public void answerIsEmptyIfCursorPointsToLastElement() {
		var validatorInfoService = setUpService();
		var result = validatorInfoService.getValidators(3, Optional.of(VALIDATOR_1));
	...
	private ValidatorInfoService setUpService() {
		...
		var particle1 = new ValidatorParticle(VALIDATOR_1, false, "V1", "http://v1.com");
		var particle2 = new ValidatorParticle(VALIDATOR_2, false, "V2", "http://v2.com");
		var particle3 = new ValidatorParticle(VALIDATOR_3, false, "V3", "http://v3.com");
