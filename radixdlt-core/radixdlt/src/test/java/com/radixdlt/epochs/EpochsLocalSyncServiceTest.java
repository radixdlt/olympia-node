...
public class EpochsLocalSyncServiceTest {
	...
	public void when_ledger_update_without_epoch_change__then_should_forward() {
		LedgerUpdate baseLedgerUpdate = mock(LedgerUpdate.class);
		EpochsLedgerUpdate ledgerUpdate = mock(EpochsLedgerUpdate.class);
		when(ledgerUpdate.getBase()).thenReturn(baseLedgerUpdate);
		...
		verify(ledgerUpdateEventProcessor, times(1)).process(ledgerUpdate.getBase());
	}
