...
import static com.radixdlt.api.archive.store.berkeley.BalanceEntry.createBalance;
...
public class ArchiveHandlerTest {
	...
	private final ArchiveHandler handler = new ArchiveHandler(
		archiveService, transactionStatusService,
		validatorInfoService, networkInfoService, 2
	);
	...
	public void testTokenBalancePositional() {
		...
		when(archiveService.getTokenBalances(any(REAddr.class)))
	...
	public void testStakePositionsPositional() {
		...
		when(archiveService.getStakePositions(any(REAddr.class)))
			.thenReturn(Result.ok(List.of(balance1, balance2, balance3)));
	...
	public void testNativeToken() {
		when(archiveService.getNativeTokenDescription())
	...
	public void testTokenInfoPositional() {
		when(archiveService.getTokenDescription(any(String.class)))
	...
	public void testTransactionHistoryPositional() {
		...
		when(archiveService.getTransactionHistory(any(), eq(5), any()))
	...
	public void testLookupTransactionPositional() {
		...
		when(archiveService.getTransaction(txId)).thenReturn(Result.ok(entry));
