...
public class MetricsControllerTest {
	private final MetricsService service = mock(MetricsService.class);
	private final MetricsController controller = new MetricsController(service);
	...
	@Test
	public void handleMetricsRequest() {
		var exchange = mock(HttpServerExchange.class);
		var sender = mock(Sender.class);

		when(exchange.getResponseHeaders()).thenReturn(new HeaderMap());
		when(exchange.getResponseSender()).thenReturn(sender);
		when(service.getMetrics()).thenReturn("# some metrics\n");

		controller.handleMetricsRequest(exchange);

		verify(sender).send("# some metrics\n");
	}
}