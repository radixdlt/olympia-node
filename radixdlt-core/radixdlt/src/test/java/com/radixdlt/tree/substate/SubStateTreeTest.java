...
public class SubStateTreeTest {
  ...
  public void t() {
    ...
    for (REStateUpdate rEStateUpdate : this.berkeleySubStateStore.getREStateUpdateList()) {
      byte[] bytes = subStateTree.get(rEStateUpdate.getId());
      Assert.assertArrayEquals(SubStateTree.getValue(rEStateUpdate.isBootUp()), bytes);
    }
    System.out.println();
  }
}
