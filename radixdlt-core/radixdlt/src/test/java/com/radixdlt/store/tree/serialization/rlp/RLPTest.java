...
import static com.radixdlt.store.tree.serialization.rlp.ByteUtil.byteArrayToInt;
import static com.radixdlt.store.tree.serialization.rlp.RlpTestData.*;
...
public class RLPTest {
   ...
   /* 2 */
		byte[] message = RLP.encodeList(new byte[]{});
		SHA3Digest sha3Digest = new SHA3Digest(256);
		byte[] uncleListHash = new byte[sha3Digest.getDigestSize()];
		if (message.length != 0) {
			sha3Digest.update(message, 0, message.length);
		}
		sha3Digest.doFinal(uncleListHash, 0);
   ...
   /* 3 */    byte[] coinbase =
        ...
        byte[] header = RLP.encodeList(
                prevHash, uncleListHash, coinbase);
...
//        2240089100000070
    ...
    public void test15() {
        ...
        String helloMsg= "f8 91 80 0b 80 b8 46 45 74 68 65 72 65 75 6d 28 2b 2b 29 2f 5a 65 72 6f 47 6f 78 2e 70 72 69 63 6b 6c 79 5f 6d 6f 72 73 65 2f 76 30 2e 34 2e 32 2f 52 65 6c 65 61 73 65 2d 57 69 6e 33 32 2f 57 69 6e 64 6f 77 73 2f 56 53 32 30 31 33 07 82 76 5f b8 40 ea 99 2c 13 68 7c 20 e7 90 a9 ff a6 df 8b 1a 16 86 88 e2 a8 87 36 5d 7a 50 21 86 fa 0d 62 20 e8 3e 11 3a 1f e7 7d c0 68 9d 55 ba 2e 8a 83 aa 8e 20 42 18 f4 d8 e7 32 82 5b d7 80 cf 94 ed 5c c3";
	...
	public void testEncodeSmallInteger() {
		Integer test = Integer.valueOf(15);
		String expected = "0f";
		byte[] encoderesult = RLP.encode(test);
		assertEquals(expected, Hex.toHexString(encoderesult));
	...
	public void testEncodeMediumInteger() {
		Integer test = Integer.valueOf(1000);
