...
public class SupportedCapability {
  ...
  private List<VersionedCapabilityWithMessages> versionedCapabilities;

  public SupportedCapability(List<String> disabledCapabilities) {
    versionedCapabilities =
        List.of(
                new VersionedCapabilityWithMessages(
                    new VersionedCapability(Capabilities.CONSENSUS, 1),
                    List.of(
                        ConsensusEventMessage.class,
                        GetVerticesRequestMessage.class,
                        GetVerticesResponseMessage.class,
                        GetVerticesErrorResponseMessage.class)),
                new VersionedCapabilityWithMessages(
                    new VersionedCapability(Capabilities.LEDGER_SYNC, 1),
                    List.of(
                ...
                new VersionedCapabilityWithMessages(
                    new VersionedCapability(Capabilities.MEMPOOL, 1),
                    List.of(MempoolAddMessage.class)),
                new VersionedCapabilityWithMessages(
                    new VersionedCapability(Capabilities.P2P, 1),
                    List.of(
                        GetPeersMessage.class,
                        PeersResponseMessage.class,
                        PeerPingMessage.class,
                        PeerPongMessage.class)))
            .stream()
            .filter(it -> !disabledCapabilities.contains(it))
            .toList();
  }
}
