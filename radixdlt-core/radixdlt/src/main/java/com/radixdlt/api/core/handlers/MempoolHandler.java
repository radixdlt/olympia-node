...
public final class MempoolHandler extends CoreJsonRpcHandler<MempoolRequest, MempoolResponse> {
  ...
  @Inject
  private MempoolHandler(CoreModelMapper coreModelMapper, RadixEngineMempool mempool) {
  ...
  @Override
  public MempoolResponse handleRequest(MempoolRequest request) throws CoreApiException {
    coreModelMapper.verifyNetwork(request.getNetworkIdentifier());

    return mempool.getData(
        map -> {
          var response = new MempoolResponse();
          map.keySet()
              .forEach(
                  txnId ->
                      response.addTransactionIdentifiersItem(
