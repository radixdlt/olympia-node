...
public final class BetanetForkTxnRulesModule extends AbstractModule {
	@ProvidesIntoMap
	@StringMapKey("betanet1")
	Function<RERulesConfig, RERules> betanetV1() {
		return config -> {
			...
			return new RERules(
				...
				fees ? new TokenFeeChecker() : (p, txn) -> { },
				View.of(maxRounds)
			);
		};
	}
	...
	@ProvidesIntoMap
	@StringMapKey("betanet2")
	Function<RERulesConfig, RERules> betanetV2() {
		return config -> {
			var maxRounds = config.getMaxRounds();
			var fees = config.includeFees();
	...
	@ProvidesIntoMap
	@StringMapKey("betanet3")
	Function<RERulesConfig, RERules> betanetV3() {
		return config -> {
			var maxRounds = config.getMaxRounds();
			var fees = config.includeFees();
	...
	Function<RERulesConfig, RERules> betanetV4() {
		return config -> {
			...
			var betanet4 = new ConstraintMachineConfig(
				...
				v4.getProcedures(),
				fees ? new FixedFeeMetering(fixedFee) : (procedureKey, param, context) -> { }
			);
