...
public class TreeUtils {
	...
	public static int[] applyPrefix(int[] rawKey, int oddPrefix, int evenPrefix) {
		var keyLength = rawKey.length;
		int[] prefixed;
		if (keyLength % 2 == 0) {
			IntBuffer ib = IntBuffer.allocate(2 + keyLength);
			ib.put(evenPrefix);
			ib.put(rawKey);
			prefixed = ib.array();
		} else {
			IntBuffer ib = IntBuffer.allocate(1 + keyLength);
			ib.put(oddPrefix);
			ib.put(rawKey);
			prefixed = ib.array();
		}
