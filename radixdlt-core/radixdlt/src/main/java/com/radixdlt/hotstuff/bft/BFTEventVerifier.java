...
package com.radixdlt.hotstuff.bft;
...
public final class BFTEventVerifier implements BFTEventProcessor {
    ...
    validAuthor(vote)
        .ifPresent(
            node -> {
              ...
              final var highQcValid =
                  measureTime(
                      () -> safetyRules.verifyHighQcAgainstTheValidatorSet(vote.highQC()),
                      CounterType.BFT_PACEMAKER_TIME_SPENT_ON_VERIFYING_CERTS);
    ...
    validAuthor(proposal)
        .ifPresent(
            node -> {
              ...
              final var highQcValid =
                  measureTime(
                      () -> safetyRules.verifyHighQcAgainstTheValidatorSet(proposal.highQC()),
                      CounterType.BFT_PACEMAKER_TIME_SPENT_ON_VERIFYING_CERTS);
  ...
  private <T> T measureTime(Supplier<T> thunk, CounterType counterType) {
    final var start = System.currentTimeMillis();
    final var res = thunk.get();
    counters.add(counterType, System.currentTimeMillis() - start);
    return res;
  }
