...
public final class AuthHandshaker {
  ...
  public Pair<byte[], AuthHandshakeResult> handleInitialMessage(ByteBuf data) {
    try {
      ...
      final var handshakeResult =
          finalizeHandshake(remoteEphemeralKey, message.getNonce(), message.getLatestForkName());
  ...
  public AuthHandshakeResult handleResponseMessage(ByteBuf data) throws IOException {
    try {
      ...
      return finalizeHandshake(remoteEphemeralKey, message.getNonce(), message.getLatestForkName());
  ...
  private AuthHandshakeSuccess finalizeHandshake(
      ECPublicKey remoteEphemeralKey, HashCode remoteNonce, Optional<String> remoteLatestForkName) {
    ...
    return AuthHandshakeResult.success(remotePubKey, secrets, remoteLatestForkName);
  }
