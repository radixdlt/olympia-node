...
public class RLP {
	 ...
	 * Support for String, Integer, BigInteger and Arrays of these types.
 	 *
	 * @param input as object or List of objects
	 * @return byte[] RLP encoded
	 */
	public static byte[] encode(Object input) {
		if (input instanceof Object[] inputArray) {
			if (inputArray.length == 0) {
				return encodeLength(inputArray.length, OFFSET_SHORT_LIST);
			}
	...
	private static boolean isList(Object input) {
		return input != null && input.getClass().isArray() && !input.getClass().getComponentType().isPrimitive();
	}
    ...
    public static byte[] encodeBigInteger(BigInteger srcBigInteger) {
    	if (Objects.equals(srcBigInteger, BigInteger.ZERO)) {
