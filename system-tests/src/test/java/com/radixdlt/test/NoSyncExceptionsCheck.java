...
public class NoSyncExceptionsCheck implements RemoteBFTCheck {
	private final long checkInterval;
	private final TimeUnit checkIntervalUnit;

	public NoSyncExceptionsCheck(long checkInterval, TimeUnit checkIntervalUnit) {
		...
		return Observable.interval(checkInterval, checkIntervalUnit)
			.map(i -> network.getNodeNames().stream()
				.map(nodeName -> network.queryJson(nodeName, "api/system")
						...
						.doOnSuccess(counters -> Assert.assertEquals("sync exception counter is zero",
