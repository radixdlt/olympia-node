...
public class TxBlobDTO {
	...
	private TxBlobDTO(byte[] blob, byte[] hashToSign) {
		this.blob = blob;
		this.hashToSign = hashToSign;
	}

	public static TxBlobDTO create(byte[] blob, byte[] hashToSign) {
		requireNonNull(blob);
		requireNonNull(hashToSign);

		return new TxBlobDTO(blob, hashToSign);
	}
	...
	public static TxBlobDTO create(
		...
		return new TxBlobDTO(Hex.decode(blob), Hex.decode(hashToSign));
	}
