...
public class SyncRadixApi extends RadixApiBase implements RadixApi {
  ...
  static Result<RadixApi> connect(
    ...
    return ofNullable(url)
        .map(
            baseUrl ->
                Result.ok(
                    new SyncRadixApi(baseUrl, primaryPort, secondaryPort, client, authentication)))
        ...
        .flatMap(
            syncRadixApi ->
                syncRadixApi
                    ...
                    .id()
                    .onSuccess(
                        networkId -> syncRadixApi.configureSerialization(networkId.getNetworkId()))
                    .map(__ -> syncRadixApi));
  }

  private <T> Result<T> call(
  ...
  private <T> Result<T> bodyHandler(
      HttpResponse<String> body, TypeReference<JsonRpcResponse<T>> reference) {
