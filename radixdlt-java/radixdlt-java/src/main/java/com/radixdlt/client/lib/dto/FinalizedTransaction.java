...
public final class FinalizedTransaction {
  ...
  private FinalizedTransaction(
      byte[] blob, ECDSASignature signature, ECPublicKey publicKey, AID txId) {
    this.blob = blob;
  ...
  public static FinalizedTransaction create(
      byte[] blob, ECDSASignature signature, ECPublicKey publicKey, AID txId) {
    requireNonNull(blob);
  ...
  public String getTxId() {
    return Optional.ofNullable(txId).map(AID::toString).orElse(null);
  }
