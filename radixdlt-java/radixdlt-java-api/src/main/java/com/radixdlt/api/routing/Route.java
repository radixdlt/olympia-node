...
public class Route<I, O> implements RouteSource {
	...
	@Override
	public RouteSource under(String prefix) {
		return new Route<>(normalize(prefix + "/" + path), endPoint);
	}
	...
	private static String normalize(String fullPath) {
		...
		var stringPath = SLASHES.matcher("/" + path).replaceAll("/");
		var index = stringPath.lastIndexOf('/');

		return index < (stringPath.length() - 1) ? stringPath + "/" : stringPath;
	}
}
