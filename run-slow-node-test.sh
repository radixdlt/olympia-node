...
container_name="${CONTAINER_NAME:-test-executor}"
echo "Container Name = $container_name"
docker build -f system-tests/docker/Dockerfile -t radix-system-test .
docker ps -a
docker rm -f "${container_name}"
docker create  --pid=host --privileged  -v /var/run/docker.sock:/var/run/docker.sock --network=host --cap-add=NET_ADMIN  --name=${container_name} radix-system-test ./gradlew clean dockerSystemTests
