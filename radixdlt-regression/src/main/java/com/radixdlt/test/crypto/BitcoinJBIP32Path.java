...
public final class BitcoinJBIP32Path implements HDPath {
  ...
  private static String toBitcoinJPath(String standardPath) {
    ...
    return standardPath.replace(
        HDPaths.BIP32_HARDENED_MARKER_STANDARD, BIP32_HARDENED_MARKER_BITCOINJ);
  }

  private static String standardizePath(String nonStandardPath) {
    // For some reason BitcoinJ chose to not use standard notation of hardened path components....
    return nonStandardPath.replace(
        BIP32_HARDENED_MARKER_BITCOINJ, HDPaths.BIP32_HARDENED_MARKER_STANDARD);
  }

  public int indexOfLastComponent() {
    if (depth() == 0) {
      throw new IllegalStateException(
          "Trying to access component of a BIP32 path with 0 depth, this is undefined.");
    }
    return depth() - 1;
  }
  ...
  @Override
  public HDPath next() {
    ArrayList<ChildNumber> nextPathComponents =
        new ArrayList<>(pathListFromBIP32Path(this, indexOfLastComponent()));
    nextPathComponents.add(
        new ChildNumber(lastComponent().num() + 1, lastComponent().isHardened()));
    org.bitcoinj.crypto.HDPath nextPath =
        new org.bitcoinj.crypto.HDPath(this.hasPrivateKey(), nextPathComponents);
    return new BitcoinJBIP32Path(nextPath);
  }

  private static List<ChildNumber> pathListFromBIP32Path(
      BitcoinJBIP32Path path, @Nullable Integer toIndex) {
