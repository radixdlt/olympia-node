...
public final class HDKeyPair {
  ...
  // Not public since one should be using `HDKeyPairDerivation.deriveKeyAtPath` instead of using
  // this directly.
  ...
  /**
   * The resulting (child) {@link ECKeyPair} after having derived it using the {@link HDPath} and
   * {@link HDKeyPairDerivation}, together with some root key (from a seed).
   *
   * @return resulting (child) {@link ECKeyPair} after having derived it using the {@link HDPath}
   *     and {@link HDKeyPairDerivation}, together with * some root key (from a seed).
   */
  ...
  public boolean equals(Object o) {
    ...
    return Objects.equal(ecKeyPair, hdKeyPair.ecKeyPair) && Objects.equal(path, hdKeyPair.path);
  }
