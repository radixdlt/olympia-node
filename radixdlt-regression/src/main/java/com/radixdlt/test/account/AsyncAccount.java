            ...
            api ->
                api.token()
                    ...
                    .map(
                        nativeToken -> {
                          var newAccount =
                              new AsyncAccount(api, ECKeyPair.generateNew(), nativeToken);
                          logger.trace(
                              "Generated new account with address: {}", newAccount.getAddress());
                          logger.trace("New account connected to {}", jsonRpcUrl);
                          logger.trace(
                              "Network's native token is {}({})",
                              nativeToken.getName(),
    ...
    return getOwnTokenBalances()
        .map(
            tokenBalancesDTO -> {
              ...
              var balances =
                  tokenBalancesDTO.getTokenBalances().stream()
