...
public class DockerConfiguration {
  ...
  public DockerConfiguration(
      ...
      boolean shouldInitializeNetwork,
      String image,
  ...
  public static DockerConfiguration fromEnv() {
    var socketUrl =
        TestingUtils.getEnvWithDefault("RADIXDLT_DOCKER_DAEMON_URL", "unix:///var/run/docker.sock");
    var containerName =
        TestingUtils.getEnvWithDefault("RADIXDLT_DOCKER_CONTAINER_NAME", "system-testing-core%d");
    if (containerName.contains("_")) {
      logger.warn("Underscores in container names ({}) should be avoided", containerName);
    }
    var shouldInitializeNetworkString =
        TestingUtils.getEnvWithDefault("RADIXDLT_DOCKER_INITIALIZE_NETWORK", "false");
    var shouldInitializeNetwork = Boolean.parseBoolean(shouldInitializeNetworkString);
    var image =
        TestingUtils.getEnvWithDefault("RADIXDLT_DOCKER_IMAGE", "radixdlt/radixdlt-core:develop");
    var initialNumberOfNodesString =
        TestingUtils.getEnvWithDefault("RADIXDLT_DOCKER_INITIAL_NUMBER_OF_NODES", "3");
    var initialNumberOfNodes = Integer.parseInt(initialNumberOfNodesString);
    var networkName =
        TestingUtils.getEnvWithDefault("RADIXDLT_DOCKER_NETWORK_NAME", "system_testing_network");
    var dockerLogin = TestingUtils.getEnvWithDefault("RADIXDLT_DOCKER_LOGIN_COMMAND", "");
    return new DockerConfiguration(
        socketUrl,
        containerName,
        shouldInitializeNetwork,
        image,
