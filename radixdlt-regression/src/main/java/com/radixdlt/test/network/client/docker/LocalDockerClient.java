...
/**
 * A tcp, socket-based client that connects to a local daemon, consuming Docker's Engine API:
...
public class LocalDockerClient implements DockerClient {
  ...
  public void connect() {
    ...
    try {
      dockerClient.pingCmd().exec();
        ...
        | NoClassDefFoundError e) { //  this happens when you try to connect to a socket via windows
  ...
  public String runCommand(String containerId, String... commands) {
    ...
    var cmdResponse =
        dockerClient.execCreateCmd(containerId).withAttachStdout(true).withCmd(commands).exec();
    try {
      dockerClient
  ...
  public void cleanup(String... parameters) {
    ...
    logger.debug(
        "Removed existing network '{}', along with its {} containers",
        networkName,
        count.intValue());
  }
}
