...
public class Staking extends RadixNetworkTest {
  ...
  public void i_observe_that_validators_have_stakes_delegated_to_them() {
    var totalDelegatedStakeAcrossNetwork =
        validatorsBuffer.stream()
            .mapToDouble(value -> Double.parseDouble(value.getTotalDelegatedStake().toString()))
            .sum();
    assertTrue(
        "No stake was found in any validator, something is wrong with the test network",
  ...
  public void i_unstake_xrd_from_the_same_validator(int tokensToUnstake) {
    account1.unstake(
        firstValidator.getAddress(), Amount.ofTokens(tokensToUnstake), Optional.empty());
  }
  ...
  public void i_receive_xrd_in_emissions() {
    var latestStakedAmount =
        account1.account().stakes(account1.getAddress()).stream()
            .filter(
                stakePositions -> stakePositions.getValidator().equals(firstValidator.getAddress()))
