...
import static com.fasterxml.jackson.dataformat.cbor.CBORConstants.BYTE_EMPTY_STRING;
import static com.fasterxml.jackson.dataformat.cbor.CBORConstants.BYTE_FALSE;
import static com.fasterxml.jackson.dataformat.cbor.CBORConstants.BYTE_FLOAT32;
import static com.fasterxml.jackson.dataformat.cbor.CBORConstants.BYTE_FLOAT64;
import static com.fasterxml.jackson.dataformat.cbor.CBORConstants.BYTE_NULL;
import static com.fasterxml.jackson.dataformat.cbor.CBORConstants.BYTE_OBJECT_INDEFINITE;
import static com.fasterxml.jackson.dataformat.cbor.CBORConstants.BYTE_STRING_1BYTE_LEN;
import static com.fasterxml.jackson.dataformat.cbor.CBORConstants.BYTE_STRING_2BYTE_LEN;
import static com.fasterxml.jackson.dataformat.cbor.CBORConstants.BYTE_TAG_BIGNUM_NEG;
import static com.fasterxml.jackson.dataformat.cbor.CBORConstants.BYTE_TAG_BIGNUM_POS;
import static com.fasterxml.jackson.dataformat.cbor.CBORConstants.BYTE_TAG_DECIMAL_FRACTION;
import static com.fasterxml.jackson.dataformat.cbor.CBORConstants.BYTE_TRUE;
import static com.fasterxml.jackson.dataformat.cbor.CBORConstants.PREFIX_TYPE_ARRAY;
import static com.fasterxml.jackson.dataformat.cbor.CBORConstants.PREFIX_TYPE_BYTES;
import static com.fasterxml.jackson.dataformat.cbor.CBORConstants.PREFIX_TYPE_INT_NEG;
...
import static com.fasterxml.jackson.dataformat.cbor.CBORConstants.PREFIX_TYPE_TAG;
import static com.fasterxml.jackson.dataformat.cbor.CBORConstants.PREFIX_TYPE_TEXT;
 ...
 * com.fasterxml.jackson.dataformat.cbor.CBORGenerator}. The only changes have been to change {@code
 * _writeChunkedString(...)} to the slower but more canonical {@code writeLongString(...)}.
 *
...
public class RadixCBORGenerator extends GeneratorBase {
  ...
  /**
   * Let's ensure that we have big enough output buffer because of safety margins we need for UTF-8
   * encoding.
   */
  static final int BYTE_BUFFER_FOR_OUTPUT = 16000;

  /**
   * Longest char chunk we will output is chosen so that it is guaranteed to fit in an empty buffer
   * even if everything encoded in 3-byte sequences; but also fit two full chunks in case of
   * single-byte (ascii) output.
   */
  private static final int MAX_LONG_STRING_CHARS = (BYTE_BUFFER_FOR_OUTPUT / 4) - 4;

  /** This is the worst case length (in bytes) of maximum chunk we ever write. */
  private static final int MAX_LONG_STRING_BYTES = (MAX_LONG_STRING_CHARS * 3) + 3;

  /** Enumeration that defines all togglable features for CBOR generator. */
  public enum Feature implements FormatFeature {
    /**
     * Feature that determines whether generator should try to use smallest (size-wise) integer
     * representation: if true, will use smallest representation that is enough to retain value; if
     * false, will use length indicated by argument type (4-byte for <code>int</code>, 8-byte for
     * <code>long</code> and so on).
     */
    WRITE_MINIMAL_INTS(true),

    /**
     * Feature that determines whether CBOR "Self-Describe Tag" (value 55799, encoded as 3-byte
     * sequence of <code>0xD9, 0xD9, 0xF7</code>) should be written at the beginning of document or
     * not.
     *
     * <p>Default value is <code>false</code> meaning that type tag will not be written at the
  ...
  /**
   * To simplify certain operations, we require output buffer length to allow outputting of
   * contiguous 256 character UTF-8 encoded String value. Length of the longest UTF-8 code point
   * (from Java char) is 3 bytes, and we need both initial token byte and single-byte end marker so
   * we get following value.
   *
   * <p>Note: actually we could live with shorter one; absolute minimum would be for encoding
   * 64-character Strings.
   */
  private static final int MIN_BUFFER_LENGTH = (3 * 256) + 2;
  ...
  private static final int INDEFINITE_LENGTH = -2; // just to allow -1 as marker for "one too many"
  ...
  /** Bit flag composed of bits that indicate which {@code CBORGenerator.Feature}s are enabled. */
  ...
  /**
   * Offset to index after the last valid index in {@link #_outputBuffer}. Typically same as length
   * of the buffer.
   */
  ...
  /**
   * Let's keep track of how many bytes have been output, may prove useful when debugging. This does
   * <b>not</b> include bytes buffered in the output buffer, just bytes that have been written using
  ...
  /**
   * Number of elements remaining in the current complex structure (if any), when writing
   * defined-length Arrays, Objects; marker {@link #INDEFINITE_LENGTH} otherwise.
   */
  ...
  public RadixCBORGenerator(
      IOContext ctxt, int stdFeatures, int formatFeatures, ObjectCodec codec, OutputStream out) {
    ...
    if (_outputEnd < MIN_BUFFER_LENGTH) {
      throw new IllegalStateException(
          "Internal encoding buffer length ("
              + _outputEnd
   ...
   * @param offset Offset pointing past already buffered content; that is, number of bytes of valid
   *     content to output, within buffer.
   */
  public RadixCBORGenerator(
      IOContext ctxt,
      int stdFeatures,
  ...
  /**
   * No way (or need) to indent anything, so let's block any attempts. (should we throw an exception
   * instead?)
   */
  @Override
  ...
  public final void writeFieldId(long size) throws IOException {
    if (_writeContext.writeFieldName(String.valueOf(size))
  ...
  private void _failSizedArrayOrObject() throws IOException {
    _reportError(
        String.format(
            "%s size mismatch: number of element encoded is not equal to reported array/map size.",
   ...
   *
   * <p>NOTE: only use this method if you really know what you are doing.
   */
  ...
  private static final int MAX_SHORT_STRING_BYTES = 23 * 3 + 2;

  private static final int MAX_MEDIUM_STRING_CHARS = 255;
  // in case it's > 255 bytes
  private static final int MAX_MEDIUM_STRING_BYTES = 255 * 3 + 3;
  ...
  /**
   * Helper method called when the whole character sequence is known to fit in the output buffer,
   * but not all characters are single-byte (ASCII) characters.
   */
  ...
  private void _throwIllegalSurrogate(int code) {
    ...
    throw new IllegalArgumentException(
        "Illegal character point (0x" + Integer.toHexString(code) + ") to output");
  }
  ...
  private final void closeComplexElement() throws IOException {
    switch (_currentRemainingElements) {
      ...
      default:
        _reportError(
            String.format(
                "%s size mismatch: expected %d more elements",
                _writeContext.typeDesc(), _currentRemainingElements));
    }
    _currentRemainingElements =
        (_elementCountsPtr == 0) ? INDEFINITE_LENGTH : _elementCounts[--_elementCountsPtr];
  }
