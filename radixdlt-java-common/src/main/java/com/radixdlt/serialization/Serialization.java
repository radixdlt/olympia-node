...
public class Serialization {
	...
	Serialization(SerializerIds idLookup, SerializationPolicy policy) {
		var availableOutputs = EnumSet.allOf(Output.class);
		...
		var hashDsonMapper = JacksonCborMapper.create(idLookup, filterProviderFor(policy.getIncludedFields(Output.HASH)), true);

		var apiDsonMapper = JacksonCborMapper.create(idLookup, filterProviderFor(policy.getIncludedFields(Output.API)),
		...
		for (var e : List.of(Output.PERSIST, Output.WIRE)) {
