...
public final class Strings {
  ...
  /**
   * Brutally convert a string to a sequence of ASCII bytes by discarding all but the lower 7 bits
   * of each {@code char} in {@code s}.
   *
   * <p>The primary purpose of this method is to implement a speedy converter between strings and
   * bytes where characters are known to be limited to the ASCII character set.
   *
   * <p>Note that the output will consume exactly {@code s.length()} bytes.
   *
  ...
  /**
   * Convert a sequence of ASCII bytes into a string. Note that no bounds checking is performed on
   * the incoming bytes &#x2014; the upper bit is silently discarded.
   *
