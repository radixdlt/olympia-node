...
public final class UInt384 implements Comparable<UInt384> {
	...
	/**
	 * A constant holding the minimum value a {@code UInt384} can
	 * have, 0.
	 */
	...
	/**
	 * Factory method for materialising an {@link UInt384} from a {@code short}
	 * value.
	 *
	...
	/**
	 * Factory method for materialising an {@link UInt384} from a {@code long} value.
	...
	/**
	 * Factory method for materialising an {@link UInt384} from an {@link UInt128}
	...
	/**
	 * Factory method for materialising an {@link UInt384}
	 * from constituent {@link UInt128} and {@link UInt256} parts.
	 *
	...
	/**
	 * Factory method for materialising an {@link UInt384} from an array
	...
	/**
	 * Factory method for materialising an {@link UInt384} from a string.
	...
	public UInt384 add(UInt384 other) {
		...
		UInt128 carry = this.low.shiftRight()
				...
				.isHighBitSet() ? UInt128.ONE : UInt128.ZERO;
	...
	public UInt384 add(UInt256 other) {
		...
		// Note that the use of method calls and the ternary operator
		// very likely precludes this from being branch-free in java.
		UInt128 newHigh = this.low.shiftRight()
				.add(other.low.shiftRight())
				.add(this.low.and(other).and(UInt256.ONE))
				.isHighBitSet() ? this.high.increment() : this.high;
		return UInt384.from(newHigh, newLow);
	}
	...
	public UInt384 subtract(UInt256 other) {
		...
		UInt128 newHigh = this.low.shiftRight()
				...
				.isHighBitSet() ? this.high.decrement() : this.high;
		return UInt384.from(newHigh, newLow);
	}
