...
public final class UInt128 implements Comparable<UInt128> {
	...
	/**
     * A constant holding the minimum value an {@code UInt128} can
     * have, 0.
     */
    public static final UInt128 MIN_VALUE = new UInt128(0x0000_0000_0000_0000L, 0x0000_0000_0000_0000L);

    /**
     * A constant holding the maximum value an {@code UInt128} can
     * have, 2<sup>128</sup>-1.
     */
    public static final UInt128 MAX_VALUE = new UInt128(0xFFFF_FFFF_FFFF_FFFFL, 0xFFFF_FFFF_FFFF_FFFFL);
	...
	/**
	 * Factory method for materialising an {@link UInt128} from a {@code short}
	 * value.
	 *
	...
	/**
	 * Factory method for materialising an {@link UInt128} from a {@code long} value.
	 * Note that values are zero extended into the 128 bit value.
	 *
	 * @param value The value to be represented as an {@link UInt128}.
	 * @return {@code value} as an {@link UInt128} type.
	 */
	...
	/**
	 * Factory method for materialising an {@link UInt128} from two {@code long}
	...
	/**
	 * Factory method for materialising an {@link UInt128} from four {@code int}
	 * values. {@code high} is the most significant word, and {@code low} the least
	 * significant.
	 *
	 * @param high The most significant word of the 128 bit value.
	...
	/**
	 * Factory method for materialising an {@link UInt128} from an array
	...
	/**
	 * Factory method for materialising an {@link UInt128} from a string.
	 * Conversion is performed base 10 and leading '+' sign character is
	 * permitted.
	 *
	...
	public UInt128 add(UInt128 other) {
		...
		long partial1 = (this.midLow & INT_MASK) + (other.midLow & INT_MASK) + (partial0 >>> Integer.SIZE);
		long partial2 = (this.midHigh & INT_MASK) + (other.midHigh & INT_MASK) + (partial1 >>> Integer.SIZE);
	...
	/**
	 * Increment a number.  Equivalent to {@code this.add(UInt128.ONE)}, but
	 * faster.
	 *
	...
	/**
	 * Decrement a number.  Equivalent to {@code this.subtract(UInt128.ONE)}, but
	 * faster.
	 *
	...
	public UInt128 multiply(UInt128 multiplicand) {
		...
		long mh = (partial20 & INT_MASK) + (partial11 & INT_MASK) + (partial02 & INT_MASK) + (ml >>> Integer.SIZE);
		long hh = (partial30 & INT_MASK) + (partial21 & INT_MASK) + (partial12 & INT_MASK) + (partial03 & INT_MASK) + (mh >>> Integer.SIZE);
	...
	/**
	 * Calculates the integer square root, that is the largest number {@code n}
	 * such that {@code n * n <= this}.
	 *
	...
	/**
	 * Shift {@code this} left n bits.  A zero bit is moved into the
	 * rightmost bits.
	 *
	...
	/**
	 * Logical shift {@code this} right n bits.  Zeros are shifted into
	 * the leftmost bits.
	 *
	...
	public UInt128 or(UInt128 other) {
		return UInt128.from(this.high | other.high, this.midHigh | other.midHigh, this.midLow | other.midLow, this.low | other.low);
	}
     ...
     * UInt128 (the number of zero bits to the right of the rightmost
     * one bit).  Returns -1 if this UInt128 contains no one bits.
