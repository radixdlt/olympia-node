...
public record UpdatingRakeReady(
    long rakeIncreaseDebounceEpochLength, EpochData epochData, ValidatorStakeData stakeData)
    implements ReducerState {
  public void update(ValidatorFeeCopy update) throws ProcedureException {
    ...
    if (rakeIncrease > ValidatorUpdateRakeConstraintScrypt.MAX_RAKE_INCREASE) {
    ...
    var epoch =
        update.epochUpdate().orElseThrow(() -> new ProcedureException("Must contain epoch update"));
    if (rakeIncrease > 0) {
