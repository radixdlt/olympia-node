...
public final class RewardingValidatorsV4 implements ReducerState {
  ...
  RewardingValidatorsV4(EpochUpdateConfig config, UpdatingEpoch updatingEpoch) {
    this.config = config;
  ...
  ReducerState next(ExecutionContext context) {
    if (validatorBFTData.isEmpty()) {
      return PreparingUnstakeV4.create(config, updatingEpoch, updatingValidators, preparingStake);
    }
  ...
  public static NavigableMap<REAddr, UInt256> createStakeMap(REAddr validatorOwner, UInt256 rake) {
    var map = new TreeMap<REAddr, UInt256>(EpochUpdateConstraintScryptV4.STAKE_COMPARATOR);
